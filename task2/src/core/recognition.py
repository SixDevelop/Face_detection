...
from core.utils import feature_extractors, load, split_data, load_mas,load_cl
...
def single_photo_recognition(
    ...
    if b:
        test_images = load_cl(test_db)
    else:
        test_images=load_mas(test_db)
    images = load(db_name)
    methods = [param[0] for param in params]
    classifiers: List[Classifier] = []
    ...
    X_train, _, y_train, _ = \
        split_data(data=images, templ_to=9)

    X_test, _, y_test, _ = \
        split_data(data = test_images, templ_to=9)
    ...
    return  [templates_for_tests, X_test]